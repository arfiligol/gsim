...
def _surface_epr_face_kinds(
    ...
    """Normalize the accepted public face selector without name inference."""
    if face_kind is None:
        return (None,)
    return (face_kind,) if isinstance(face_kind, str) else tuple(map(str, face_kind))


def _surface_epr_assignments(
    interfaces: Mapping[str, Mapping[str, Any]] | Sequence[Mapping[str, Any]] | None,
) -> tuple[dict[str, Any], ...]:
    """Validate explicit A/B Surface-EPR selections before meshing."""
    if interfaces is None:
        return ()
    records = (
        [
            {"interface_type": name, **dict(cast(Mapping[str, Any], value))}
            for name, value in interfaces.items()
        ]
        if isinstance(interfaces, Mapping)
        else [dict(value) for value in interfaces]
    )
    ...
    for record in records:
        ...
        try:
            ...
            raise ValueError(
                f"Surface EPR interface {interface_type!r} requires a preset."
            ) from None
        preset_name = record.pop("preset_name", interface_type.lower())
        face_kind = record.pop("face_kind", None)
        role = record.pop("role", "boundary_surface")
        required = record.pop("required", True)
        if not isinstance(preset_name, str) or not preset_name:
            raise ValueError("Surface EPR preset_name must be a non-empty string.")
        if not isinstance(role, str) or not role:
            raise ValueError("Surface EPR role must be a non-empty string.")
        if not isinstance(required, bool):
            raise TypeError("Surface EPR required must be a boolean.")
        if record:
            raise ValueError(
                "Unknown Surface EPR interface fields: "
                + ", ".join(sorted(str(key) for key in record))
            )
        declared_type = preset.get("interface_type")
        if declared_type not in {None, interface_type}:
            raise ValueError(
                f"Surface EPR preset {preset_name!r} declares interface_type "
                f"{declared_type!r}, not {interface_type!r}."
            )
        preset.setdefault("interface_type", interface_type)
        assignments.append(
            {
...
class PalaceSimMixin:
    ...
    simulation_type: Literal[
        "driven", "eigenmode", "electrostatic", "boundarymode", "magnetostatic"
    ]
    ...
    def set_surface_epr(
        ...
        *,
        ...
        """Select optional structured SGB Route A or B Surface-EPR geometry.

        Native upstream meshing remains the default.  The typed route is an
        opt-in boundary because physical names are display-only: SGB provides
        stable surface ownership, adjacency, component and net provenance.
        """
        route = representation.upper()
        if route not in {"A", "B"}:
        ...
        object.__setattr__(
            ...
            "_surface_epr_config",
            {
                ...
                "interfaces": _surface_epr_assignments(interfaces),
            },
        )

    def _surface_epr_postprocessing(self) -> Any | None:
        """Build Route A/B dielectric rows from structured mesh provenance."""
        config = getattr(self, "_surface_epr_config", {})
        interfaces = config.get("interfaces")
        if not interfaces:
            return None
        mesh_result = getattr(self, "_last_mesh_result", None)
        if mesh_result is None or mesh_result.manifest is None:
            raise ValueError(
                "Surface EPR postprocessing requires a structured SGB mesh."
            )
        ...
        route = config["representation"]
        catalog = build_interface_surface_catalog(mesh_result.groups)
        specs = []
        for assignment in interfaces:
            interface_type = assignment["interface_type"]
            surfaces = tuple(
                surface
                for surface in catalog.surfaces
                if surface.representation == route
                and surface.interface_type == interface_type
            )
        ...
        return build_postprocessing_config_from_manifest(
            mesh_result.manifest, dielectric_interfaces=tuple(specs)
        )
    ...
    def activate_substrate(
        ...
        *,
        ...
        """Activate one typed substrate layer for an SGB Route A/B scene."""
    ...
    def activate_inter_die_vacuum(
        ...
        *,
        ...
        """Activate the typed inter-die vacuum layer for SGB Route A/B."""
        self._activate_region(
            layer,
    ...
    def activate_outer_vacuum(
        ...
        *,
        ...
        """Activate symmetric typed outer vacuum for SGB Route A/B."""
        self._activate_region(
            layer,
            role="outer_vacuum",
            margin_x=margin_x,
    ...
    def _activate_region(
        ...
        layer: str,
        *,
        role: Literal["substrate", "inter_die_vacuum", "outer_vacuum"],
        **kwargs: Any,
    ) -> None:
        """Store deterministic typed region activation without native fallback."""
        if self._airbox_config:
            raise ValueError("Explicit SGB regions cannot be mixed with set_airbox().")
        from gsim.palace.models.regions import ActivatedRegion

        region = ActivatedRegion(layer=layer, role=role, **kwargs)
        regions = dict(getattr(self, "_activated_regions", {}))
        existing = regions.get(region.layer)
        if existing is not None and existing.role != region.role:
            raise ValueError(
                f"Stack layer {region.layer!r} is already activated as "
                f"{existing.role!r}."
            )
        regions[region.layer] = region
        object.__setattr__(self, "_activated_regions", regions)

    def _activated_region_values(self) -> tuple[Any, ...]:
        """Return SGB region activation in deterministic layer-name order."""
        regions = getattr(self, "_activated_regions", {})
        return tuple(regions[name] for name in sorted(regions))
    ...
    def validate_config(self) -> ValidationResult:
        ...
        if self.simulation_type == "electrostatic":
            ...
            errors.extend(
                ...
                if terminal.net_id is None and not terminal.layer
            )
    ...
    def _generate_mesh_internal(
        ...
        mesh_result = generate_mesh(
            ...
            palace_version=DEFAULT_PALACE_CONFIG_VERSION,
            ...
            surface_epr_representation=getattr(self, "_surface_epr_config", {}).get(
                "representation"
            ),
    ...
    def preview(
        ...
        *,
        ...
        if getattr(self, "_surface_epr_config", None):
            raise ValueError(
                "Surface EPR Route A/B preview is not supported; use mesh("
                "show_gui=True) so preview follows the structured SGB path."
            )
    ...
    def mesh(
        ...
        *,
        ...
        Returns:
            SimulationResult with mesh path and summary fields. Rich mesh
            provenance remains available on ``_last_mesh_result``.
    ...
    def write_config(
        ...
        *,
        ...
        palace_version: str = DEFAULT_PALACE_CONFIG_VERSION,
        ...
        postprocessing = (
            self._surface_epr_postprocessing()
            if getattr(self, "_surface_epr_config", None)
            else None
        )
        ...
        config_path = gen_write_config(
            ...
            postprocessing_config=(
                None if postprocessing is None else postprocessing.domains
            ),
            boundary_postprocessing_config=(
                None if postprocessing is None else postprocessing.boundaries
            ),
        )

        manifest = getattr(self._last_mesh_result, "manifest", None)
        if manifest is not None:
            ...
            index_entries: list[PostprocessingIndexEntry] = list(
                () if postprocessing is None else postprocessing.index_map.entries
            )
            if self.simulation_type == "electrostatic":
                terminal_rows = config.get("Boundaries", {}).get("Terminal", [])
                terminal_map = build_terminal_index_map_from_manifest(
                    manifest,
                    terminal_rows if isinstance(terminal_rows, list) else (),
                    terminal_names=tuple(terminal.name for terminal in terminals or ()),
                )
                index_entries.extend(terminal_map.entries)
            elif self.simulation_type == "magnetostatic":
                current_rows = config.get("Boundaries", {}).get("SurfaceCurrent", [])
                current_sources = getattr(self, "current_sources", None) or ()
                current_map = build_surface_current_index_map_from_manifest(
                    ...
                    current_source_names=tuple(
                        source.name for source in current_sources
                    ),
                )
        ...
        object.__setattr__(self, "_last_palace_version", palace_version)
        return config_path
    ...
    def write_slurm_sbatch_handoff(
        ...
        **sbatch_kwargs: Any,
        ...
        """Render a canonical Slurm handoff without submitting it."""
        if self._output_dir is None:
            raise ValueError("Output directory not set. Call set_output_dir() first.")
    ...
    def generate_handoff_package(
        ...
        *,
        ...
        """Write current inputs, then delegate handoff assembly to Run Stage."""
        if self._output_dir is None:
            raise ValueError("Output directory not set. Call set_output_dir() first.")
        if write_config:
            self.write_config(
            ...
            palace_version = getattr(self, "_last_palace_version", palace_version)
        from gsim.palace.run.handoff import generate_palace_handoff_package

        package_metadata = dict(metadata or {})
        package_metadata["palace_config_version"] = palace_version
    ...
    def run_local(
        ...
        *,
        ...
        """Run through the canonical local runner, then parse legacy results."""
        if self._output_dir is None:
            raise ValueError("Output directory not set. Call set_output_dir() first.")
        if use_apptainer:
            import os

            roots = (Path.cwd(), Path(self._output_dir), Path(self._output_dir).parent)
        ...
        files = run_palace_local(
            ...
            verbose=verbose,
            palace_version=getattr(
                self, "_last_palace_version", DEFAULT_PALACE_CONFIG_VERSION
            ),
        )
