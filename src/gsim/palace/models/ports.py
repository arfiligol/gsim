...
class TerminalConfig(BaseModel):
    ...
    A terminal is either a legacy layer selector or an exact structured
    conductor-net selector. The latter is consumed only by the optional SGB
    Route A/B handoff; physical names and bounding boxes are not authority.
    """
    ...
    @model_validator(mode="after")
    def _validate_selection_mode(self) -> Self:
        """Require exactly one selector authority."""
        if self.net_id is not None:
            if not self.net_id.strip():
                raise ValueError("Terminal net_id must be non-empty.")
            if any(
                value is not None
                for value in (
                    self.layer,
                ...
                raise ValueError(
                    "Semantic-net terminals require layer, center, port_name, and "
                    "physical_label to be absent."
                )
            return self
        if not self.layer:
            raise ValueError("Terminal selector requires layer when net_id is absent.")
        return self
