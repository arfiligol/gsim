...
class PalaceSweepPointSpec:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        for field_name in _SWEEP_POINT_PATH_FIELDS:
            value = getattr(self, field_name)
            if value is not None:
                row[field_name] = _path_value(value)
        return row
        ...
        for line in stream:
            ...
            if match := _PALACE_ESTIMATED_MEMORY_RE.search(stripped):
                ...
                estimated_peak_memory[scope] = {
                    ...
                    f"{key}_bytes": _parse_resource_memory_bytes(match.group(key))
            ...
            if current_table and (match := _RESOURCE_TABLE_ROW_RE.match(stripped)):
                ...
                if current_table == "timing":
                    ...
                    row = {
                        ...
                        "per_node_bytes": _parse_resource_memory_bytes(
                            match.group("a")
                        ),
                        "total_bytes": _parse_resource_memory_bytes(match.group("b")),
                        "total_hwm_bytes": _parse_resource_memory_bytes(
                            match.group("c")
                        ),
                    }
    ...
    *,
    ...
    for table_name, (path, rows, columns) in table_specs.items():
        ...
        table_refs[table_name] = {
            "path": _relative_path_or_name(path, run_root),
            "row_count": len(rows),
        }
...
def _ordered_amr_pass_row(row: Mapping[str, Any]) -> dict[str, Any]:
    return {column: row.get(column) for column in _PALACE_AMR_PASS_COLUMNS}
...
def _parse_resource_memory_bytes(value: Any) -> float | None:
    if value is None:
        return None
...
def _sweep_point_spec_row(
    ...
    if isinstance(point, PalaceSweepPointSpec):
        ...
        row = {str(key): _json_ready(value) for key, value in point.items()}
    else:
        msg = "Sweep point specs must be mappings or PalaceSweepPointSpec instances"
        raise TypeError(msg)

    point_slug = _optional_str(row.get("point_slug"))
    if not point_slug:
    ...
    if not isinstance(parameters, Mapping):
        ...
        raise TypeError(msg)
    row["parameters"] = {
        str(key): _json_ready(value) for key, value in parameters.items()
    }
